...
export async function sendCodexMessage({
  ...
  for (const account of accounts) {
    ...
    try {
      ...
      const statusResult = await runCodexStatus({
        runCodex,
        model,
        codexHomePath: accountManager.codexHomePath,
        logger,
      ...
      results.push({
        ...
        statusResponse: statusResult?.output || null
      });
...
async function runCodexStatus({ runCodex, model, codexHomePath, logger, account }) {
  try {
    const statusResult = await runCodex({
      prompt: '/status',
      model,
      codexHomePath,
      promptMode: 'raw',
    ...
    await logger?.info('Codex status received', {
      provider: 'codex',
      accountId: account.id,
      accountEmail: account.email,
      response: statusResult.output,
      durationMs: statusResult.durationMs,
      usage: statusResult.usage,
      reasoning: {
        hasReasoning: statusResult.hasReasoning,
        reasoningItemCount: statusResult.reasoningItemCount
      }
    });
    ...
    await logger?.error('Codex status failed', {
      provider: 'codex',
      accountId: account.id,
      accountEmail: account.email,
      errorCode: result.errorCode || 'process_error',
      error: result.stderr || result.stdout || error.message,
      usage: result.usage || null
    });
