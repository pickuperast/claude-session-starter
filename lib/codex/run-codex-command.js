export async function runCodexCommand({
  ...
  try {
    const turn = await thread.run(buildPrompt({ prompt, taskMode, promptMode }), { signal: controller.signal });
...
export function buildPrompt({ prompt, taskMode = 'hard', promptMode = 'goal' } = {}) {
...
async function loadCodexSdk() {
  try {
    ...
    throw new Error(`Failed to load @openai/codex-sdk: ${getErrorMessage(error)}`);
  }
}
